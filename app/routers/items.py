...
async def create_new_user(request: CreateUserRequest, db: Session = Depends(get_db)):
    ...
    new_user = User(id=user_id, name=request.name, user_name=request.userName)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
...
async def get_user_information_by_user_name(userName: str, db: Session = Depends(get_db)):
...
async def set_story_name(request: SetStoryNameRequest, db: Session = Depends(get_db)):
    story = db.query(Story).filter(Story.id == request.storyId).first()
    if not story:
...
@router.get("/getUserStories", response_model=UserStoriesResponse)
async def get_user_stories(userId: str, maxEntries: Optional[int] = 50, db: Session = Depends(get_db)):
    stories = db.query(Story).filter(Story.user_id == userId).limit(maxEntries).all()
...
async def get_story_by_id(userId: str, storyId: str, db: Session = Depends(get_db)):
    story = db.query(Story).filter(Story.id == storyId, Story.user_id == userId).first()
    if not story:
        raise HTTPException(status_code=404, detail="Story not found")
...
async def update_images_by_text(request: UpdateImagesByTextRequest, db: Session = Depends(get_db)):
    story = db.query(Story).filter(Story.id == request.storyId, Story.user_id == request.userId).first()
    if not story:
        raise HTTPException(status_code=404, detail="Story not found")
